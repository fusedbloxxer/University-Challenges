...
void TestUtil::intOperations()
{
	BigInteger sum, var;
	char operation;

	for (;;) {
		std::cout << "(x - exit) Enter [+-]a [+-*<] [+-]b : ";

		std::cin.get(); operation = std::cin.get();
		if (operation == 'x') {
			return;
		}
...
void TestUtil::vecOperations()
{
	for (int var;;) {
		std::cout << "1. Produs scalar\n2. Maxim absolut\n3. Exit\n:";
		std::cin >> var;
